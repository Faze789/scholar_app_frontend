...
class _AllUniEventsScreenState extends State<AllUniEventsScreen> {
  ...
  Future<void> fetchAllEvents() async {
    try {
      final comsatsRes = await http.get(Uri.parse("http://192.168.100.149:5000/api/comsats_events"));
      final neduetRes = await http.get(Uri.parse("http://192.168.100.149:5000/api/neduet_events"));
      final uetRes = await http.get(Uri.parse("http://192.168.100.149:5000/api/uet_taxila_events"));

      if (comsatsRes.statusCode == 200 &&
          neduetRes.statusCode == 200 &&
          uetRes.statusCode == 200) {
        final comsatsEvents = List<Map<String, dynamic>>.from(json.decode(comsatsRes.body))
            .asMap()
            .map((index, e) {
              String link = e['link']?.toString() ?? '';
        ...
        final neduetEvents = List<Map<String, dynamic>>.from(json.decode(neduetRes.body))
            .asMap()
            .map((index, e) {
              if (index < 2) {
                return MapEntry(index, {
        ...
        final uetEvents = List<Map<String, dynamic>>.from(json.decode(uetRes.body))
            .asMap()
            .map((index, e) {
              String link = e['link']?.toString() ?? '';
              if (link.isEmpty || !link.startsWith('http')) {
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              "Select how you want to open the link:",
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        actions: [
          IconButton(
            onPressed: () {
              context.go('/events-applied', extra: widget.alumniData);
            },
                              ...
                              return Column(
                                ...
                                children: [
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      gradient: LinearGradient(
                                        colors: [uniColor, uniColor.withOpacity(0.8)],
                                        begin: Alignment.topLeft,
                                        end: Alignment.bottomRight,
                                      ),
                                      borderRadius: BorderRadius.circular(16),
                                      boxShadow: [
                                        BoxShadow(
                                          color: uniColor.withOpacity(0.3),
                                          blurRadius: 10,
                                    ...
                                    child: Row(
                                      children: [
                                        CircleAvatar(
                                          ...
                                          radius: 20,
                                          child: Icon(uniIcon, color: Colors.white, size: 24),
                                        ),
                                  ...
                                  if (events.isEmpty)
                                    Padding(
                                      ...
                                      child: Center(
                                        child: Text(
                                          "No events available for $uniName",
                                          style: TextStyle(
                                            color: Colors.grey.shade600,
                                            fontSize: 16,
                                            fontWeight: FontWeight.w500,
  ...
  Widget _buildEventCard(Map<String, dynamic> event, Color uniColor, bool isUpcoming) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        border: isUpcoming
            ? Border.all(color: uniColor.withOpacity(0.5), width: 1.5)
            : Border.all(color: Colors.grey.shade200, width: 1),
        boxShadow: [
          BoxShadow(
            color: isUpcoming ? uniColor.withOpacity(0.15) : Colors.grey.shade200,
            blurRadius: 12,
            offset: const Offset(0, 4),
          ),
        ],
            ...
            child: Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    CircleAvatar(
                      backgroundColor: isUpcoming ? uniColor.withOpacity(0.1) : Colors.grey.shade100,
                      radius: 24,
                      child: Icon(
                        isUpcoming ? Icons.event_available : Icons.event,
                        color: isUpcoming ? uniColor : Colors.grey.shade600,
                        size: 24,
                      ),
                    ),
                    const SizedBox(width: 12),
                              ...
                              Expanded(
                                child: Text(
                                  ...
                                  style: TextStyle(
                                    fontSize: 18,
                                    fontWeight: FontWeight.bold,
                                    color: isUpcoming ? uniColor : Colors.grey.shade800,
                                  ),
                                ),
                              ),
                ...
                if (event['description'] != null && event['description'].toString().isNotEmpty) ...[
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: isUpcoming ? uniColor.withOpacity(0.05) : Colors.grey.shade50,
                      borderRadius: BorderRadius.circular(12),
                    ),
                    child: Text(
                      event['description'],
                  ...
                  children: [
                    ...
                    if (hasLink)
                      ElevatedButton.icon(
                        ...
                        label: Text(
                          isUpcoming ? "Details" : "View Event",
                          style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14),
                        ),
                        style: ElevatedButton.styleFrom(
                          backgroundColor: Colors.white,
                          foregroundColor: uniColor,
                          side: BorderSide(color: uniColor.withOpacity(0.5)),
                          padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 16),
                          shape: RoundedRectangleBorder(
                            borderRadius: BorderRadius.circular(12),
                          ),
